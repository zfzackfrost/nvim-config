...
local function gen_modeline(t)
  local modeline_str = string.format(
    ...
    utils.func.iff(vim.bo.expandtab, 'et', 'noet'),
    vim.bo.filetype,
    vim.bo.shiftwidth,
    vim.bo.tabstop,
    vim.bo.softtabstop
  )
  modeline_str = utils.vim.comments.format(modeline_str)

  local first_line = nvim.buf_get_lines(0, 0, 1, false)[1]
  local has_shebang = utils.str.starts_with(first_line, '#!')
  local line_nr = utils.func.iff(has_shebang, 1, 0)
