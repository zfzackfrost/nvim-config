...
---@brief
---
--- Modified by: Zachary Frost
--- ]]
local M = {}

local tbl_path = function(t, keys)
  if keys == nil or (type(keys) == 'table' and #keys == 0) then
    return t
  end
  if type(t) ~= 'table' then
    error('Failed to index table: Input is not a table', 6)
  end
  for i, k in ipairs(keys) do
    t = t[k]
    if t == nil then
      error(('Failed to index table using `keys` list: Missing key `%s` (i.e. `keys[%d]`)'):format(k, i), 6)
    end
  end
  return t
end
  ...
  return setmetatable({}, {
    __index = function(self, key)
      if rawget(self, 'mod') == nil then
        rawset(self, 'mod', require(require_path))
      end
      return self.mod[key]
    end,
...
---
--- </pre>
M.require_on_exported_call = function(require_path, keys)
  return setmetatable({}, {
    __index = function(self, key)
      return function(...)
        if rawget(self, 'mod') == nil then
          rawset(self, 'mod', tbl_path(require(require_path), keys))
        end
        return self.mod[key](...)
      end
    end,
